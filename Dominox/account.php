		...
		<div class="container">
			<div class="col-xs-12">
				<div class="col1" width="1000">
					<h4 style="color:#cc3300" class="text-left "> MY ACCOUNT </h4>
					<hr>
						<?php
						try
						{
							...
							$stmt = $dbh->prepare("SELECT * FROM `games` 
								WHERE `username1` = :username OR `username2` = :username ORDER BY `date` LIMIT 10");
							$stmt->bindParam(':username', $username, PDO::PARAM_STR);
							$stmt->execute();
							$user_games = $stmt->rowCount();
							if($user_games == 0)
							{
								$message = 'There are no games to display.<br>You can <a href = "play.php">PLAY HERE</a>.';
							}
								else
								{
									echo '<h3>'.'Games history'.'</h3>';
									...
									echo'</table>';
									echo '<p>Play again <a href = "play.php"> HERE</a>.</p>';
									$stmt = null;
								}
						}
							catch(Exception $e)
